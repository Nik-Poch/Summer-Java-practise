...
public class Generator
{
    ...
    public Cell[][] generate_map(int x, int y)
    {
        ...
        for (int i = 0; i<4; i++)
        {
            rand = random.nextInt(100 ) + 1;/**Рандомим 1 или 2 руды с вероятностью 70% в большую сторону*/
            if (rand>30)
            {
            ...
            {
                rand_x = random.nextInt((x / 2) + curr_x) + curr_x;/**Радномим точку спауна руды*/
                rand_y = random.nextInt((x / 2) + curr_y) + curr_y;
                rand_ore =  random.nextInt(4) + 1;/**Радномим тип руды*/
